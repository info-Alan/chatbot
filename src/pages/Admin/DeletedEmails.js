...
function DeletedEmails({handleLogout}) {
  ...
  const checkDeletedEmails = async () => {
    setSyncing(true);
    setSyncProgress(0);
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/check_deleted_emails`);
      ...
      if (data.success) {
        ...
        console.error('Error checking deleted emails:', data.message);
      }
    } catch (error) {
  ...
  const fetchDeletedEmails = async () => {
    try {
      const response = await fetch(`${API_URL}/get_deleted_emails`);
      const data = await response.json();
      if (data.success) {
        setDeletedEmails(data.data);
  ...
  const handleSearch = (event) => {
    ...
    setSearchTerm(term);
    const filtered = deletedEmails.filter(email =>
      ...
      <div className="dashboard-main">
        ...
        <main className="content">
          <section className="email-section">
            <h2 className="section-title">Deleted Emails</h2>
            ...
            {syncing && (
              <div className="syncing-bar-container">
                ...
                <p className="syncing-text">
                  Checking for deleted emails... ({syncProgress}%)
                </p>
              </div>
            )}
            ...
            <div className="controls-container">
              <input
                ...
                placeholder="Search deleted emails by sender, subject, or date..."
                value={searchTerm}
            ...
            <div className="email-table">
              {loading ? (
                <div className="loading">Loading deleted emails...</div>
                ...
                <div className="no-data">No deleted emails found.</div>
              )}
            </div>
          </section>
